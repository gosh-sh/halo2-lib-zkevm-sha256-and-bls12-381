use crate::{
    ...
    sha256::component::{
        circuit::shard::{Sha256ComponentShardCircuit, Sha256ComponentShardCircuitParams},
...
#[test]
fn test_mock_sha256_shard_circuit_raw_outputs() {
    let k: usize = 14;
    ...
    let inputs = vec![
        ...
        vec![],
        (0u8..1).collect::<Vec<_>>(),
        (0u8..55).collect::<Vec<_>>(), // single block (55 + 1 + 8 = 64 bytes fits in one block)
        (0u8..56).collect::<Vec<_>>(), // forces 2 blocks
        (0u8..200).collect::<Vec<_>>(),
    ];

    let mut params =
        Sha256ComponentShardCircuitParams::new(k, num_unusable_row, capacity, publish_raw_outputs);
    let base_circuit_params =
        Sha256ComponentShardCircuit::<Fr>::calculate_base_circuit_params(&params);
    params.base_circuit_params = base_circuit_params;
    let circuit =
        Sha256ComponentShardCircuit::<Fr>::new(inputs.clone(), params.clone(), false);
...
#[test]
fn test_prove_sha256_shard_circuit_raw_outputs() {
    ...
    let mut circuit_params =
        Sha256ComponentShardCircuitParams::new(k, num_unusable_row, capacity, publish_raw_outputs);
    let base_circuit_params =
        Sha256ComponentShardCircuit::<Fr>::calculate_base_circuit_params(&circuit_params);
    circuit_params.base_circuit_params = base_circuit_params;
    let circuit =
        Sha256ComponentShardCircuit::<Fr>::new(inputs, circuit_params.clone(), false);
    ...
    let circuit =
        Sha256ComponentShardCircuit::<Fr>::new(inputs, circuit_params, true);
...
#[test]
fn test_mock_sha256_shard_circuit_commit() {
    let k: usize = 14;
    let num_unusable_row: usize = 109;
    let capacity: usize = 15;
...
#[test]
fn test_prove_sha256_shard_circuit_commit() {
    let _ = env_logger::builder().is_test(true).try_init();
    ...
    let break_points = circuit.base_circuit_break_points();
    let circuit =
        Sha256ComponentShardCircuit::<Fr>::new(inputs.clone(), circuit_params.clone(), true);
    circuit.set_base_circuit_break_points(break_points);
