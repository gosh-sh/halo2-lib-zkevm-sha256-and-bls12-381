...
/// Real prover test for BLS12-381 signature verification at degree 19.
#[test]
fn bench_bls_signature_real_prover_k19() {
    let bench_params = BlsSignatureCircuitParams {
    ...
    let k = bench_params.degree;
    println!("========== BLS12-381 Signature Verification Real Prover (k={k}) ==========");
    ...
    println!("  VKey generation:   {:?}", stats.vk_time.time.elapsed());
    println!("  PKey generation:   {:?}", stats.pk_time.time.elapsed());
    println!("  Proof generation:  {:?}", stats.proof_time.time.elapsed());
    println!("  Proof size:        {} bytes", stats.proof_size);
    println!("  Proof verification:{:?}", stats.verify_time.time.elapsed());
    println!("  Circuit params:    {:?}", stats.config_params);
